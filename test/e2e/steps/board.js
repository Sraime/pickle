...
function addStepsFromDataTable(steps) {
  ...
  steps.rows.forEach((step) => {
    ...
    I.fillField(
      { css: BoardSectionLocators.sectionStepsAddLocator(sectionName) },
      stepName
    );
    I.pressKey("Enter");
  });
...
Given(
  "le step {string} de la section {string} est en mode édition",
  (step, sectionName) => {
...
When(
  "j'ajoute le step suivant à la section {string}",
...
When(
  "j'ajoute les steps suivant à la section {string}:",
  (sectionName, steps) => {
    var stepName;
    steps.rows.forEach((step) => {
...
When(
  "je supprime le step {string} de la section {string}",
  async (step, sectionName) => {
    let btnLocator = {
      xpath:
        ".//section-steps[contains(@class, 'section-steps-" +
...
When(
  "j'active le mode d'édition pour le step {string} de la section {string}",
  () => {
    let stepElement = {
      xpath:
...
Then(
  "la section {string} contient les steps dans l'ordre suivant:",
  (sectionName, steps) => {
    var stepName;
    steps.rows.forEach((step) => {
      stepName = step.cells[0].value;
...
Then(
  "le step {string} n'existe pas la section {string}",
  (step, sectioName) => {
