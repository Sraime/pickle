...
describe("FeatureUpdaterService", () => {
  ...
  const subjectSynchroEvent: Subject<FeatureUpdateData> = new Subject<
    FeatureUpdateData
  >();

  mockSocketSynchroService.getFeatureUpdateObservable.mockReturnValue(
    subjectSynchroEvent
  );
  beforeEach(() =>
  ...
  describe("synchro enabled", () => {
    ...
    it("should dispatch to the synchroniser", () => {
      ...
      expect(
        mockSocketSynchroService.dispatchFeatureUpdate
      ).toHaveBeenCalledWith(featureUpdateTypeCreate);
    });
  });
