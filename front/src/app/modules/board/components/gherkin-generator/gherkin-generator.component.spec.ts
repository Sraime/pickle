...
describe('GherkinGeneratorComponent', () => {
	...
	const sectionsName = ['Given', 'When', 'Then'];
	...
	configureTestSuite(() => {
		...
		TestBed.configureTestingModule({
			...
			providers: [{ provide: SectionServiceService, useValue: sectionService }]
		});
	});
	...
	it('should not display then code by default', () => {
		const sectionCode = fixture.debugElement.query(By.css('.section-code'));
		expect(sectionCode).toBeFalsy();
	});

	it('should update the generation state when sections are updated', () => {
		const spyUpdate = jest.spyOn(component, 'updateGenerationState');
	...
	sectionsName.forEach((sectionName, index) => {
		describe('section code ' + sectionName, () => {
			it(
				'should display the gherkin code in the ' + index + ' position',
				async(() => {
					sectionService.getSectionObservable.mockReturnValue(of(defaultSectionValue));
					component.ngOnInit();
					fixture.whenStable().then(() => {
						fixture.detectChanges();
						const sectionCode = fixture.debugElement.queryAll(By.css('.section-code'));
						expect(sectionCode[index].nativeElement.textContent.includes(sectionName + ' step')).toBeTruthy();
					});
				})
			);
			...
			it('should not generate the code and display an error if the section is not valid', async(() => {
				...
				fixture.whenStable().then(() => {
					fixture.detectChanges();
					const code = fixture.debugElement.query(By.css('.section-code'));
					expect(code).toBeFalsy();
					const sectionCode = fixture.debugElement.query(By.css('.section-code-error'));
					expect(sectionCode.nativeElement.textContent).toEqual(
			...
			it('should display the 2 steps returned by the service', async(() => {
				...
				fixture.whenStable().then(() => {
					fixture.detectChanges();
					const stepCode = fixture.debugElement.queryAll(
						By.css('.section-code:nth-child(' + (index + 1) + ') .step-code')
					);
					expect(stepCode[0].nativeElement.textContent.includes(sectionName + ' step1')).toBeTruthy();
					expect(stepCode[1].nativeElement.textContent.includes('And step2')).toBeTruthy();
				});
			}));
		});
	});
